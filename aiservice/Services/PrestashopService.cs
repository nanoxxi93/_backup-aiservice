...
namespace AIService.Services
{
    public class PrestashopService
    {
        ...
        public static async Task<JObject> GetResource(AppSettings appSettings, RequestInDTO requestInDTO, string resource, Dictionary<string, object> data)
        {
        ...
        public static async Task<JObject> PostResource(AppSettings appSettings, RequestInDTO requestInDTO, string resource, Dictionary<string, object> data)
        {
            ...
            try
            {
                ...
                JObject searchresult = await GetResource(appSettings, requestInDTO, resource, search);
                if (searchresult[resource] != null && searchresult[resource].HasValues)
                {
                    data["id"] = searchresult[resource][0]["id"].ToString();
                    result = await PutResource(appSettings, requestInDTO, resource, data);
                }
                else
                {
                    // New Element
        ...
        public static async Task<JObject> PutResource(AppSettings appSettings, RequestInDTO requestInDTO, string resource, Dictionary<string, object> data)
        {
